...
@time begin
    intermediate = ForwardDiff.derivative(x -> fuzzy_correlator(ra .* x .^2, x*dec, fuzzy_shear_one, fuzzy_shear_two, initial_centers, initial_weights, 3, 10.0, 10, 100.0, spacing="linear", fuzziness=0.00005, dist_metric=Vincenty_Formula, tol=1e-6, verbose=false, max_iter=1000)[1], 90.0)
end
